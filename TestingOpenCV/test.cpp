...
int old_bicubic_interprolation(Mat srcimage,float scale){
  ...
  for (int each_Rowindex_in_matDst1 = 0; each_Rowindex_in_matDst1 < matDst1.rows; ++each_Rowindex_in_matDst1)
  {
      float unknownfloat_y = (float)((each_Rowindex_in_matDst1) * scale_y);
      int srcimage_y = cvFloor(unknownfloat_y);
      unknownfloat_y -= srcimage_y;
      srcimage_y = std::min(srcimage_y, srcimage.rows - 3);
      srcimage_y = std::max(1, srcimage_y);
      ...
      for (int i = 0; i < matDst1.cols; ++i)
      {
          ...
          for (int k = 0; k < srcimage.channels(); ++k)
           {
              matDst1.at<cv::Vec3b>(each_Rowindex_in_matDst1, i)[k] = abs((srcimage.at<cv::Vec3b>(srcimage_y-1, sx-1)[k] * coeffsX[0] * coeffsrcimage_y[0] + srcimage.at<cv::Vec3b>(srcimage_y, sx-1)[k] * coeffsX[0] * coeffsrcimage_y[1] +
